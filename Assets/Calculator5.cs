...
public class Calculator5 : MonoBehaviour
{
    ...
    void CalculateFullProbability()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            P_Bi[i] = GetValidatedRelativeProbability(GetBiInputField(i).text);
        ...
        if (P_A == -1)
        {
            ...
            if (invalidState)
            {
                errorText.text = "Invalid input. Can't calculate P(A) if not all data is given." + P_Bi[0] + " " + P_Bi[1]+ " " + relativeSum;
                return;
            }
            else
            {
    ...
    double GetValidatedRelativeProbability(string input)
    {
        double value = 0;
        if (string.IsNullOrEmpty(input))
        {
        ...
        errorText.text = "Input value cant be negative number.";
        return -2;
    }
